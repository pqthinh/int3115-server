...
Mark.getAll = (result) => {
  sql.query(
    "SELECT * FROM tindaluu t LEFT JOIN user u on t.id_nguoiluutin = u.id LEFT JOIN tindang td on t.id_tindang = td.id_tindang",
    (err, res) => {
      if (err) {
...
Mark.remove = (mark, result) => {
  sql.query(
    "DELETE FROM tindaluu WHERE id_tindang = ? AND id_nguoiluutin = ? ",
    [mark.id_tindang, mark.id_nguoiluutin],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
...
Mark.create = (newMark, result) => {
  sql.query("INSERT INTO tindaluu SET ?", newMark, (err, res) => {
    ...
    sql.query(
      "SELECT * FROM tindaluu t LEFT JOIN user u on t.id_nguoiluutin = u.id LEFT JOIN tindang td on t.id_tindang = td.id_tindang ORDER BY t.id DESC LIMIT 1",
      (err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
...
Mark.search = (req, result) => {
  ...
  var qr =
    "SELECT * FROM tindaluu t JOIN tindang td ON t.id_tindang = td.id_tindang join user u on u.id  = t.id_nguoiluutin WHERE 1 ";
